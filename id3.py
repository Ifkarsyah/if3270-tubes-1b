    ...
    if isinstance(attribute, str):
        ...
        for attr in single_attribute:
            ...
            if split:
                ...
                split_entropy += (-1.0) * ((len(s_attr) * 1.0 / size)
                                           * log2(len(s_attr) * 1.0 / size))
        ...
        for vi in values:
            ...
            if examples.empty:
                ...
                new_node = id3_build_tree(
                    examples_vi, goal_values, target_attribute, next_attr)

                for i in range(len(goal_values)):
                    children_label[goal_values[i]
                                   ] += new_node.children_label[goal_values[i]]
...
def ID3(df):
    ...
    id3_tree = id3_build_tree(
        data_example, goal_values, target_attribute, attributes)
